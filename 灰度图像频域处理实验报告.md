...
6.  探究 Sobel 算子核大小（3、7、11）对边缘检测结果的影响，分析平滑性、边缘粗细与噪声抑制的权衡。
...
​	基于傅里叶变换的微分性质与卷积性质，在频域构造水平差分滤波器 \(H_x\) 和垂直差分滤波器 \(H_y\)，分别与频谱相乘后逆变换得到水平梯度和垂直梯度，再合成梯度幅值图。同时可视化差分滤波器的频率响应（热力图）。
...
图 3 `house.bmp`图像的一阶差分滤波结果

![](./imgs/differential_filter_result_2.png) 
图 4 AI生成条纹图像验证差分方向选择性
...
​    		<img src="./imgs/hpf_vs_sobel.png" style="zoom: 33%;" />  
 图 5 频域高通滤波与 Sobel 边缘检测对比
...
<img src="./imgs/sobel_kernel_comparison.png" style="zoom: 33%;" />

图 6 不同 Sobel 核大小对比（ksize=3, 7, 11）
...
​	频域滤波需要选择截止频率 \(D_0\)。若直接给定固定半径，例如 \(D_0=30\) 像素，则在不同的图像尺寸或内容下含义不一致，导致滤波强度不稳定。因此，本实验采用径向能量累计法自适应选取截止频率，并进一步提出基于频谱斜率与图像熵的动态阈值方法，形成递进式优化方案。

### 5.1 方案一：径向能量累计法（本次实验默认为 95%）

#### 算法步骤

1. 计算频移后频谱的幅度平方 \(|F(u,v)|^2\)（能量谱）。  
2. 以频率中心为原点，统计每个半径 \(r\) 上的总能量 \(E(r) = \sum_{D(u,v)\in[r,r+1)} |F(u,v)|^2\)。  
...
- **优点**：计算简单，自适应图像尺寸，保证保留 95% 的能量主成分。  
- **局限**：对所有图像使用统一的能量阈值（95%），忽略了图像内容本身（平滑或纹理丰富）对截止频率真实需求的影响；对于纹理极其丰富的图像，95% 阈值可能使 \(D_0\) 过大，导致低通滤波过保留高频噪声；对于非常平滑的图像，95% 阈值可能使 \(D_0\) 过小，导致滤波效果微弱。

### 5.2 方案二：基于频谱斜率与图像熵的动态自适应法

#### 改进思想

​	不同图像的频谱能量衰减速度不同，纹理丰富的图像能量衰减慢（频谱平坦），平滑图像能量衰减快（频谱陡峭）。同时，灰度熵可以量化图像的纹理复杂度。将这两个特征结合，动态调整能量累积百分比，使 \(D_0\) 随图像内容自适应变化。

#### 算法步骤

1. 计算频谱径向能量分布 \(E(r)\) 及能量累计曲线 \(C(r)\)（同方法一）。  
2. 计算**频谱斜率** \(\alpha\)：对径向能量 \(E(r)\) 进行对数域线性拟合，取中段半径（例如 \(r=5\) 到 \(r=80\)）的斜率绝对值。\(\alpha\) 越大表示能量衰减越快，图像越平滑。  
3. 计算**图像灰度熵** \(H\)：\(H = -\sum_{i=0}^{255} p_i \log_2 p_i\)，其中 \(p_i\) 为灰度概率。熵越高，纹理越复杂。  
4. 动态确定能量百分比：
   \[
   \text{percent} = 0.85 + 0.1 \tanh(\alpha - 0.5) + 0.05 \tanh\left(\frac{H - 7}{2}\right)
   \]
   并限制在 \([0.80, 0.97]\) 之间。对于高通滤波可额外增加 0.03。  
5. 找到满足 \(C(r) \ge \text{percent}\) 的最小半径作为 \(D_0\)。
...
def compute_spectral_slope(fft_shifted):
    ...
    if total > 0:
        radial_energy /= total
    r_start, r_end = 5, min(80, max_r - 5)
    if r_end <= r_start:
        return 0.5
    r_vals = np.arange(r_start, r_end)
    y_vals = np.maximum(radial_energy[r_start:r_end], 1e-6)
...
def compute_cutoff_frequency_adaptive(fft_shifted, img_uint8, filter_type='lowpass'):
    ...
    return float(cutoff_idx[0]) if len(cutoff_idx) > 0 else float(max_r)
```

#### 优点与局限

- **优点**：充分利用图像自身的频谱特性和统计特征，实现内容自适应的截止频率选择；对平滑图像压缩低频（保留较少高频），对纹理丰富图像保留更多高频，使滤波效果更符合人眼视觉。  
- **局限**：需要额外计算图像熵和对数域拟合，计算量略高于方法一；经验公式中的调参需要针对不同图像集进行微调，泛化性需进一步验证。

### 5.3 两种方法对比

| 方法                | 能量阈值策略                                            | 图像特征利用                     | 计算复杂度 | 适用场景                                   |
| ------------------- | ------------------------------------------------------- | -------------------------------- | ---------- | ------------------------------------------ |
| 径向能量累计 95%    | 固定 95% 能量累计                                       | 仅频谱径向能量                   | 低         | 通用，作为基准方法                         |
| 基于频谱斜率+图像熵 | 动态百分比（0.80~0.97），随能量衰减速度和纹理复杂度变化 | 频谱形状（斜率）+ 灰度分布（熵） | 中等       | 对图像内容敏感，需要精细控制高频保留程度时 |

**递进关系**：方法一建立了自适应截止频率的基本框架，解决了固定半径对尺寸敏感的问题；方法二在此基础上引入图像内容特征，进一步优化了能量阈值的选取，使滤波器能够根据图像的平滑程度和纹理丰富度自动调节通带宽度，实现了从“尺寸自适应”到“内容自适应”的优化升级。

在本次实验中，默认采用方法二（自适应法），同时保留了方法一作为可选参数（`--cutoff_method energy`），便于对比验证。实验表明，对于纹理丰富的图像（如 `house.bmp` 中屋顶瓦片），方法二会略微提高百分比，保留更多高频细节；对于平滑的背景区域，方法二则适当压低百分比，使滤波效果更柔和。

---

## 6 实验结论

1. 灰度图像可以看作二维离散信号，其频率组成可以通过二维傅里叶变换进行分析。频谱中心化后低频能量集中，高频对应边缘、纹理和噪声。

2. 理想滤波器概念简单，但频率响应突变容易带来振铃效应；高斯滤波器通过平滑过渡可以实现无振铃的低通/高通滤波，更适合实际图像处理。

3. 低通滤波可以平滑图像但会削弱边缘和纹理细节；高通滤波可以增强边缘和细节，但也可能放大噪声。两者通过截止频率 D0*D*0 控制滤波强度。

4. 基于傅里叶变换微分性质的频域一阶差分滤波器能够精确提取图像的水平梯度、垂直梯度和梯度幅值，具有明确的方向选择性，但无平滑环节，对噪声敏感。

5. Sobel 算子通过局部梯度计算实现边缘检测，相比普通高通滤波，边缘结构更集中。核大小体现了边缘定位、噪声抑制和细节保留之间的折中：小核定位准但噪声敏感，大核抗噪强但边缘粗。

6. Canny 算法通过多阶段筛选和细化（高斯平滑、非极大值抑制、双阈值连接）得到单像素宽、连续的二值边缘，在噪声抑制和边缘定位上优于 Sobel，适用于高质量边缘提取任务。

7. 截止频率的自适应选择是频域滤波的关键。本实验实现了两种递进方案：

   - **方案一（径向能量累计）**：根据频谱能量分布自动确定 D0*D*0，解决了固定半径对图像尺寸的依赖，保证了 95% 的能量主成分被保留。

   - **方案二（基于频谱斜率与图像熵的动态自适应）**：在方法一基础上引入频谱能量衰减斜率（反映图像平滑程度）和灰度熵（反映纹理复杂度），动态调整能量累计百分比（0.80~0.97），使截止频率能够根据图像内容自动优化。对于平滑图像，百分比偏低，截止半径较小，滤波效果更强；对于纹理丰富图像，百分比偏高，截止半径较大，保留更多高频细节。实验表明方法二在内容差异性大的图像集上具有更好的适应性。

   采用自适应截止频率后，滤波器不再依赖固定经验参数，能够根据图像自身的频谱和统计特征自动调整，大幅提高了频域滤波算法的鲁棒性与普适性。
